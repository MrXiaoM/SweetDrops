...
public class Event {
    ...
    Event(SweetDrops plugin, ConfigurationSection config, String id) {
        this.id = id;
        ConfigurationSection section;

        this.whiteListWorlds = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
        this.whiteListWorlds.addAll(config.getStringList("worlds"));
        this.whiteListWorlds.addAll(config.getStringList("worlds-whitelist"));
        ...
        for (String s : config.getStringList("tools")) {
            ...
            plugin.warn("[" + id + "] tools 输入的工具无效: " + s);
        }
        this.enchantments = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
        ...
        this.enchantmentsToInventory = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
        ...
        this.bannedEnchantments = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
        ...
        this.requirePreferredTool = config.getBoolean("require-preferred-tool", false);
        this.cancelAll = config.getBoolean("cancel.all", false);
        this.cancelIfDropAny = config.getBoolean("cancel.if-drop-any", false);
        this.overflowDisappear = config.getBoolean("overflow-disappear", false);
        this.permToInventory = config.getString("perm-to-inventory");
        ...
        for (int i = 0; i < stringList.size(); i++) {
            ...
            if (split.length < 3) {
                plugin.warn("[" + id + "] items[" + i + "] 的格式不正确，参数数量过少");
                continue;
            }
            String type = split[0];
            ...
            if (rate == null) {
                plugin.warn("[" + id + "] items[" + i + "] 指定的倍率格式不正确: " + split[1]);
                continue;
            }
            String item = split[2];
            ...
            if (type.equalsIgnoreCase("mc")) {
                ...
                plugin.warn("[" + id + "] items[" + i + "] 指定的原版物品不存在: " + item);
                continue;
            }
            ...
            plugin.warn("[" + id + "] items[" + i + "] 的格式不正确，未找到合适的物品提供器");
        }
        ...
        if (section != null) for (String key : section.getKeys(false)) {
            ...
            if (level == null) {
                plugin.warn("[" + id + "] fortune.multiples 指定的等级 " + key + " 不正确");
                continue;
            }
            String multipleStr = section.getString(key);
            DoubleRange multipler = getDoubleRange(multipleStr).orElseGet(() -> {
                plugin.warn("[" + id + "] fortune.multiples." + level + " 指定的倍率 " + multipleStr + " 不正确");
    ...
    public static Event load(ConfigurationSection config, String id) {
        return load(SweetDrops.getInstance(), config, id);
    }

    public static Event load(SweetDrops plugin, ConfigurationSection config, String id) {
        return new Event(plugin, config, id);
    }
